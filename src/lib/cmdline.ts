...
function getInt(value: string): number {
  // parse the value as a decimal number (with base 10)
  ...
  if (parsed < 0) {
    throw new commander.InvalidArgumentError("Delay cannot be negative.");
  }
 ...
 */
export function parse(callback?: (cmd: commander.Command) => void): commander.OptionValues {
